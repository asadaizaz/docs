...
- Human input resumes a paused case through `POST /cases/{id}/input`, after which Offload re-evaluates the thread and may immediately send the next reply.
...
Not every `FAILED` case is billable usage. In the current product, delivery failures such as bounces, complaints, and rejected sends are excluded from billable finished-case counts.
