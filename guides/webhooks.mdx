...
export async function handleOffloadWebhook(req: Request) {
  ...
  switch (event.event_type) {
    case "case.completed":
      ...
      // use event.data.attachments to look up any inbound files
      break;
    case "case.failed":
...
- If you need to fetch a file, use the attachment id from `event.data.attachments` to request a fresh download link from Offload.
