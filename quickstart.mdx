...
<Info>
  `POST /cases` is currently rate-limited by plan usage. `SANDBOX` supports up to `100` created or finished cases per UTC calendar month. `PRODUCTION` currently has a `2500` case soft cap per UTC calendar month.
</Info>
