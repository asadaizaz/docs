...
<Info>
  Offload currently enforces monthly case caps when you create a case. The current limits are `100` created or finished cases per UTC calendar month on `SANDBOX`, and a `2500` case soft cap per UTC calendar month on `PRODUCTION`.
</Info>
...
| `429` | `USAGE_LIMIT_REACHED` | Your monthly plan limit for created or finished cases has been reached |
