...
Use `GET /cases/{id}/attachments/{attachmentId}` when you need a fresh download link for a file listed in `attachments`.
...
- `attachments` as the list of inbound files seen on the case
...
Attachment download links are not stored directly on the case because the underlying AgentMail URLs expire. Use `GET /cases/{id}/attachments/{attachmentId}` to mint a fresh link when needed.
